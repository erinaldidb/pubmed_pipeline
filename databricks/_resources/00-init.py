...
dbutils.widgets.text("PUBMED_CATALOG", "pubmed_pipeline", "Catalog")
dbutils.widgets.text("PUBMED_SCHEMA_STAGE", "stage", "Schema")

reset_all_data = dbutils.widgets.get("reset_all_data") == "true"
catalog = dbutils.widgets.get("PUBMED_CATALOG")
PUBMED_CATALOG = catalog
dbName = dbutils.widgets.get("PUBMED_SCHEMA_STAGE")
PUBMED_SCHEMA_STAGE = dbName
...
if reset_all_data:
  print(f'clearing up db {PUBMED_SCHEMA_STAGE}')
  spark.sql(f"DROP DATABASE IF EXISTS `{PUBMED_CATALOG}`.`{PUBMED_SCHEMA_STAGE}` CASCADE")
...
assert PUBMED_CATALOG not in ['hive_metastore', 'spark_catalog']
#If the PUBMED_CATALOG is defined, we force it to the given value and throw exception if not.
if len(PUBMED_CATALOG) > 0:
  current_catalog = spark.sql("select current_catalog()").collect()[0]['current_catalog()']
  if current_catalog != PUBMED_CATALOG:
    catalogs = [r['catalog'] for r in spark.sql("SHOW CATALOGS").collect()]
    if PUBMED_CATALOG not in catalogs:
      spark.sql(f"CREATE CATALOG IF NOT EXISTS {PUBMED_CATALOG}")
      if PUBMED_CATALOG == 'dbdemos':
        spark.sql(f"ALTER CATALOG {PUBMED_CATALOG} OWNER TO `account users`")
  use_and_create_db(PUBMED_CATALOG, PUBMED_SCHEMA_STAGE)

if PUBMED_SCHEMA_STAGE == 'dbdemos':
  try:
    spark.sql(f"GRANT CREATE, USAGE on DATABASE {PUBMED_SCHEMA_STAGE}.{PUBMED_SCHEMA_STAGE} TO `account users`")
    spark.sql(f"ALTER SCHEMA {PUBMED_SCHEMA_STAGE}.{PUBMED_SCHEMA_STAGE} OWNER TO `account users`")
  except Exception as e:
    print("Couldn't grant access to the schema to all users:"+str(e))    

print(f"using catalog.database `{catalog}`.`{PUBMED_SCHEMA_STAGE}`")
spark.sql(f"""USE `{PUBMED_CATALOG}`.`{PUBMED_SCHEMA_STAGE}`""")    
